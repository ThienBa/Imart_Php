        ...
        <div class="main-content fl-right">
            <div class="section" id="list-product-wp">
                <div class="section-head clearfix">
                    <h3 class="section-title fl-left">Msi</h3>
                    <div class="filter-wp fl-right">
                        <p class="desc">Hiển thị <?php echo $num_msi; ?> trên <?php echo $total_row; ?> sản phẩm</p>
                ...
                <div class="section-detail">
                    <?php if(!empty($list_msi) && !isset($_POST['btn_search']) && empty($_POST['select'])){ ?>
                    <ul class="list-item clearfix">
                        <?php foreach($list_msi as $item){ ?>
                        <li>
                    ...
                    <?php if(!empty($search_high_to_low_msi) && isset($_POST['btn_search']) && $_POST['select'] == 1){ ?>
                        <p>Hiển thị <b><?php echo $total_row; ?></b> sản phẩm được sắp xếp theo giá từ <b> cao xuống thấp </b></p>
                    <ul class="list-item clearfix">
                        <?php foreach($search_high_to_low_msi as $item){ ?>
                            <li>
                            <a href="<?php echo $item['url_detail']; ?>" title="" class="thumb">
                                <img src="<?php echo $item['product_thumb'];?>">
                            </a>
                    ...
                    <?php if(!empty($search_low_to_high_msi) && isset($_POST['btn_search']) && $_POST['select'] == 2){ ?>
                        <p>Hiển thị <b><?php echo $total_row; ?></b> sản phẩm được sắp xếp theo giá từ <b> thấp lên cao </b></p>
                    <ul class="list-item clearfix">
                        <?php foreach($search_low_to_high_msi as $item){ ?>
                            <li>
                            <a href="<?php echo $item['url_detail']; ?>" title="" class="thumb">
                                <img src="<?php echo $item['product_thumb'];?>">
                            </a>
            ...
            <div class="section" id="paging-wp">
                <?php
                    if($total_row > $num_per_page){
                        echo get_pagging($num_page, $page, "?mod=product&action=msi");
                    }
                ?>
            </div>
        </div>
